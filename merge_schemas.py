    ...
    for key, value in schema.iteritems():
        ...
        if type(value) is list:
            ...
            if not(len(value)>0 and type(value[0]) is dict and len(value[0]) == 0):
                updated_schema[key] = [update_data_on_branch_recursively( value[0], '.'.join(branch.split('.')[1:]), new_value )]
...
secondary_branches = get_branches_from_schema_recursively(secondary_schema)
...
json.dump(new_schema_with_derived_datatypes, args.merged_schema, indent=4)
